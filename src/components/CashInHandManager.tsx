...
import './CashInHandManager.css'

const CashInHandManager = () => {
  ...
  const cashAccounts = bankAccounts.filter((acc) => acc.accountType === 'cash')
  ...
  const handleAddAccount = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const account: Omit<BankAccount, 'id' | 'createdAt'> = {
      ...
      accountNumber: formData.get('accountNumber') as string || 'CASH',
  ...
  return (
    <div className="cash-in-hand-manager">
      <div className="section-header">
        <h2>Cash in Hand</h2>
        <div className="header-info">
          <span className="total-cash">Total: {formatCurrency(totalCash)}</span>
      ...
      {isFormOpen && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h3>{editingAccount ? 'Edit' : 'Add'} Cash Account</h3>
            <form onSubmit={handleAddAccount}>
              <div className="form-group">
                <label>Account Name *</label>
                <input name="name" defaultValue={editingAccount?.name} required placeholder="e.g., Wallet, Home Cash" />
              </div>
              <div className="form-group">
      ...
      <div className="accounts-grid">
        {cashAccounts.length === 0 ? (
          <div className="no-accounts">
            <p>No cash accounts yet. Add one to get started!</p>
          </div>
        ) : (
            ...
            return (
              <div key={account.id} className="account-card">
                <div className="account-header">
                  <div>
                    <h3>{account.name}</h3>
                    <p className="account-type">Cash in Hand</p>
                  </div>
            ...
            <form onSubmit={handleAddTransaction}>
              ...
              <div className="form-group">
                ...
                <div className="category-select-wrapper">
                  <select name="category" id="category-select" defaultValue={editingTransaction?.category} required>
                    {(editingTransaction ? 
...
export default CashInHandManager
